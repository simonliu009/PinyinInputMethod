...
- (instancetype)init {
    ...
    if (self) {
        ...
        _queryCache.countLimit = 500;  // 最多缓存 500 个拼音的查询结果
        _cacheLimit = 500;
    }
    return self;
}
...
- (void)initializeDatabases {
    ...
    NSLog(@"[DictionaryManager] 词库初始化完成，共 %ld 条词条", (long)[_mainDB totalWordCount]);
}
...
- (void)loadMinimalBaseDictionary {
    NSLog(@"[DictionaryManager] 加载内置最小词库...");
    
    // 常用单字及其拼音（约 3000 个常用字的核心子集）
    NSArray *baseWords = @[
        // 高频单字
        @{@"word": @"的", @"pinyin": @"de", @"frequency": @1000000},
        @{@"word": @"一", @"pinyin": @"yi", @"frequency": @900000},
        @{@"word": @"是", @"pinyin": @"shi", @"frequency": @850000},
        @{@"word": @"不", @"pinyin": @"bu", @"frequency": @800000},
        @{@"word": @"了", @"pinyin": @"le", @"frequency": @780000},
        @{@"word": @"在", @"pinyin": @"zai", @"frequency": @750000},
        @{@"word": @"人", @"pinyin": @"ren", @"frequency": @720000},
        @{@"word": @"有", @"pinyin": @"you", @"frequency": @700000},
        @{@"word": @"我", @"pinyin": @"wo", @"frequency": @680000},
        @{@"word": @"他", @"pinyin": @"ta", @"frequency": @660000},
        @{@"word": @"这", @"pinyin": @"zhe", @"frequency": @640000},
        @{@"word": @"中", @"pinyin": @"zhong", @"frequency": @620000},
        @{@"word": @"大", @"pinyin": @"da", @"frequency": @600000},
        @{@"word": @"来", @"pinyin": @"lai", @"frequency": @580000},
        @{@"word": @"上", @"pinyin": @"shang", @"frequency": @560000},
        @{@"word": @"国", @"pinyin": @"guo", @"frequency": @540000},
        @{@"word": @"个", @"pinyin": @"ge", @"frequency": @520000},
        @{@"word": @"到", @"pinyin": @"dao", @"frequency": @500000},
        @{@"word": @"说", @"pinyin": @"shuo", @"frequency": @480000},
        @{@"word": @"们", @"pinyin": @"men", @"frequency": @460000},
        // 常用双字词
        @{@"word": @"我们", @"pinyin": @"wo men", @"frequency": @450000},
        @{@"word": @"中国", @"pinyin": @"zhong guo", @"frequency": @440000},
        @{@"word": @"可以", @"pinyin": @"ke yi", @"frequency": @430000},
        @{@"word": @"什么", @"pinyin": @"shen me", @"frequency": @420000},
        ...
        @{@"word": @"所以", @"pinyin": @"suo yi", @"frequency": @360000},
        ...
        @{@"word": @"开始", @"pinyin": @"kai shi", @"frequency": @230000},
    ...
    NSLog(@"[DictionaryManager] 最小基础词库加载完成，共 %lu 条", (unsigned long)baseWords.count);
}
          ...
          completionHandler:(void(^)(BOOL success, NSInteger importedCount))completionHandler
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (!words || words.count == 0) {
            NSLog(@"[DictionaryManager] 搜狗词库解析失败或为空");
        ...
        NSLog(@"[DictionaryManager] 搜狗词库导入完成，共 %lu 条", (unsigned long)taggedWords.count);
...
- (BOOL)exportDictionaryToPath:(NSString *)filePath source:(DictionarySource)source {
    ...
    [content appendString:@"# 格式：词语 拼音 词频\n\n"];
...
- (void)clearCache {
    ...
    NSLog(@"[DictionaryManager] 查询缓存已清空");
}

- (void)warmUpCache {
    NSLog(@"[DictionaryManager] 开始预热缓存...");
    
    // 加载最高频的拼音对应的词条到缓存
    // 查询词频最高的前 2000 个词条，提取其拼音加入缓存
    ...
    NSLog(@"[DictionaryManager] 缓存预热完成，已加载 %ld 个拼音查询结果", (long)warmed);
}
